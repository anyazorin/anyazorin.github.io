...
domain = "anyazorin.com"
generic_username = "anyazorin"
...
# Pass 1: collect all posts so posts_by_folder is complete before any rendering
posts_by_folder = {}
for post_folder in post_folders:
    post_files = os.listdir(f"posts/{post_folder}")
    posts = [get_post(post_folder, f) for f in post_files]
    posts_by_folder[post_folder] = sorted(posts, key=lambda x: x["order"])

# Expose to all Jinja templates (used by layout.html TOC)
env.globals["posts_by_folder"] = posts_by_folder

# Pass 2: render individual pages and list partials
lists = {}
for post_folder, posts in posts_by_folder.items():
    for post in posts:
        try:
            write_output(
...
rendered = index.render(lists=lists, posts_by_folder=posts_by_folder, name=name, title=name)
soup = bs(rendered)
for item in seotags:
    soup.head.append(bs(item))
